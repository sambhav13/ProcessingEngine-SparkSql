			...
			if(direction.equals("inbound")){
				for (Row row : l) {
					...
					if(agrDirMatchValue==3||agrDirMatchValue==2){
						...
						resultSet.put(row.get(3).toString(), row.get(1).toString()+seprator+row.get(6).toString());//KEY: TRUNKID
					}
		...
		case ApplicationConstants.AGREEMENT_DATATYPE:
			if(direction.equals("inbound")){
				for (Row row : l) {
					...
					if(agrDirMatchValue==3||agrDirMatchValue==2){
						resultSet.put(row.get(2).toString(),row.get(3).toString()+seprator+row.get(4).toString());
